...
def tfidf_features(X_test, lang):
    tfidf = None
    if lang == ARABIC:
        tfidf = pickle.load(open('models/tfidf_ar.pkl', 'rb'))
    else:
        tfidf = pickle.load(open('models/tfidf_en.pkl', 'rb'))
...
def summarize_category(input_text, statements, model_name, ar_text=False):
    ...
    if ar_text:
        features_test = tfidf_features(input_text_arr, ARABIC)
    else:
        features_test = tfidf_features(input_text_arr, ENGLISH)
